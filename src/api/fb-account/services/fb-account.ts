  ...
  ({ strapi }) => ({
    async findUserForActivity() {
      ...
      const data = await strapi.db
        .query('api::fb-account.fb-account')
        .findMany({
          where: {
    ...
    async createVisionProfile(id) {
      ...
      const fbAccount = await strapi.db
        .query('api::fb-account.fb-account')
        .findOne({
          where: { documentId: id },
        });
      ...
      if (fbAccount.visionProfileId) {
        return { visionProfileId: fbAccount.visionProfileId };
      }
    ...
    async syncVisionProfile(id) {
      ...
      const profile = await axios
        .get<any>(
          `https://v1.empr.cloud/api/v1/folders/${folderId}/profiles/${fbAccount.visionProfileId}`,
          {
            headers: visionHeaders,
          }
        )
      ...
      console.log(
        `Fetched cookies for vision profile: ${profile.id}`,
        cookies.data
      );
    ...
    async syncVisionProfileAndDelete(id) {
      ...
      return { message: 'Account synced and deleted' };
    },
    ...
    async bulkImport({ fileContent, fieldsMapping, separator }) {
      try {
        ...
        const accounts = lines
          .map((line: string) =>
            this.parseAccountLine(line, separator, fieldsMapping)
          )
        ...
        for (let i = 0; i < accounts.length; i++) {
          ...
          const existingAccount = await strapi.db
            .query('api::fb-account.fb-account')
            .findOne({
          ...
          try {
            ...
            console.error(
              `Failed to import account ${account.email || account.login}:`,
